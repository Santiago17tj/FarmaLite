	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="add-order.php" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
	...
	if(row) {
		...
		if(productId == "") {
            ...
            $("#productTable tbody tr").each(function() {
                ...
                if(!currentId) return;
                var currentRow = currentId.substring(3);
                if(currentRow != row && $("#productName"+currentRow).val() == productId) {
                    isDuplicate = true;
            ...
            if (isDuplicate) {
                // Consolida en la fila existente
