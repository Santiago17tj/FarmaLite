...
if (!$checkOpen->fetchColumn()) {
    echo "<script>alert('¡Atención! Debe ABRIR CAJA antes de facturar.'); window.location.href='apertura.php';</script>";
    exit;
}
...
<button type="button" id="btnQuickPrice" class="btn btn-info" style="position:fixed;bottom:24px;right:24px;z-index:999;box-shadow:0 2px 8px rgba(0,0,0,.2);" title="Consulta rápida de precio (F2)">
  <i class="fa fa-barcode"></i> F2 — Consultar precio
</button>

<div class="modal fade" id="quickPriceModal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><i class="fa fa-search"></i> Consulta rápida de precio</h4>
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
      </div>
      <div class="modal-body">
        <p class="text-muted">Escanea o escribe el código EAN-13. La factura actual no se pierde.</p>
        <input type="text" class="form-control input-lg" id="modalBarcodeLookup" placeholder="Código de barras..." autocomplete="off" />
        <div id="modalLookupError" class="alert alert-danger mt-3" style="display:none;"></div>
        <div id="modalLookupResult" class="mt-3" style="display:none;">
          <table class="table table-bordered table-sm">
            <tr><th>Medicina</th><td id="modalResName"></td></tr>
            <tr><th>Código</th><td id="modalResBarcode"></td></tr>
            <tr><th>Precio de venta</th><td><strong id="modalResSale" class="text-success"></strong></td></tr>
            <tr><th>Precio de compra</th><td id="modalResPurchase"></td></tr>
            <tr><th>Ganancia</th><td><strong id="modalResProfit" class="text-primary"></strong></td></tr>
            <tr><th>Stock</th><td id="modalResQty"></td></tr>
          </table>
          <button type="button" class="btn btn-success btn-block" id="modalAddToInvoice">
            <i class="fa fa-plus"></i> Agregar a la factura
          </button>
        </div>
      </div>
    </div>
...
<script src="custom/js/barcode-invoice.js"></script>

<script>
  var manageOrderTable;

  $(document).ready(function() {
    // POS UI Simplification: Hide unnecessary fields
    $('#clientName').closest('.col-sm-4').prev('label').hide();
    $('#clientName').closest('.col-sm-4').hide();
    $('#clientContact').closest('.col-sm-4').prev('label').hide();
    $('#clientContact').closest('.col-sm-4').hide();
    $('#discount').closest('.form-group').hide();
    $('#paymentType').closest('.form-group').hide();
    $('#paymentStatus').closest('.form-group').hide();
    $('#paymentPlace').closest('.form-group').hide();
    ...
    if (divRequest == 'add') {
      ...
      $("#createOrderForm").unbind('submit').bind('submit', function(event) { event.preventDefault();
        var form = $(this);
              ...
              success: function(response) {
                ...
                if (response.success == true) {
                  ...
                  $(".success-messages").html('<div class="alert alert-success">' +
                    ...
                    '<a href="add-order.php" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Agregar Nueva Factura </a>' +
      ...
      $("#editOrderForm").unbind('submit').bind('submit', function(event) { event.preventDefault();
        // alert('ok');
        var form = $(this);
  ...
  function printOrder(orderId = null) {
    if (orderId) {
      window.location.href = 'invoiceprint.php?id=' + orderId + '&print=1';
    }
  } // /print order function
